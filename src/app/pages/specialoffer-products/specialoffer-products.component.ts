...
export class SpecialOfferProductsComponent  implements OnInit,OnDestroy {
  ...
  ngOnInit(): void {
   ...
   if(currentPage != null){
    ...
    this.updateMetaTagSrv.getSeoContent('Special Offer Products').subscribe(
      (data: any) => {
        if (data.meta.status) {
          this.updateMetaTagSrv.updateMetaKeywords(data.data.title,data.data.description,data.data.keywords,"https://onbuyon.in/specialofferproducts",data.data.imageUrl)
        }
      }
    )
    this.seoService.updateCanonicalUrl('https://onbuyon.in/specialofferproducts')
    }
  }
